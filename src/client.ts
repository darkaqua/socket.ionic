...
export const getClientSocket = (
  { url, reconnect = true, reconnectIntents = 10, reconnectInterval = 1_000, silent = false }:
    Props,
) => {
		...
		socket.addEventListener(
			...
			() => {
				...
				if(reconnect && reconnectIntents > reconnects) {
					reconnects++;
					!silent && console.log(`(${reconnects}/${reconnectIntents}) Reconnecting to ${url} in ${reconnectInterval}ms...`)
					setTimeout(async () => {
						await connect();
						resolve();
					}, reconnectInterval);
